...
import Pyro5.api
from WebSocketSever import WebSocketServer
...
def custom_serializer(obj):
    if isinstance(obj, timedelta):
        # Convert timedelta to a custom dict with a marker
        return {'__timedelta__': True, 'days': obj.days, 'seconds': obj.seconds, 'microseconds': obj.microseconds}
...
class ProjectController(ConfigTreeNode, PLCControler):
    ...
    def __init__(self, frame, logger):
        ...
        self.DebugTicks = []
        self.StatusTimerId = None
        self.DispatchDebugValuesTimerId = None
        ...
        self.websocket_server = WebSocketServer(host='0.0.0.0', port=8765, interval=0.5)

        self.debug_lock = threading.Lock()


    def add_timer(self, callback, interval, one_shot=False):
        """
        ...
        Parameters:
            ...
            one_shot: Whether to execute only once (default False)  
        Returns:  
            Timer ID (can be used to remove the timer)  
        """
        with self.condition:
            timer_id = self.next_id
            self.next_id += 1
            trigger_time = time.time() + interval
            heapq.heappush(self.timers, (trigger_time, interval, callback, one_shot, timer_id))
            self.timer_ids[timer_id] = True  # Mark as active
    ...
    def remove_timer(self, timer_id):
        ...
        Args:  
            timer_id: The ID of the timer to be removed  
        Returns:  
            bool: Whether the removal was successful  
        """  
        with self.condition:
            if timer_id not in self.timer_ids:
                return False
            
            # Mark as removed
            self.timer_ids[timer_id] = False
            self.condition.notify()
            return True
        
    def _cleanup_timers(self):
        """Clean up timers marked for removal"""
        active_timers = []
        for timer in self.timers:
            *_, timer_id = timer
            if self.timer_ids.get(timer_id, False):
    ...
    def start_timers(self):
        """Start timer system"""
        if not self.running and (self.worker_thread is None or not self.worker_thread.is_alive()):
            self.running = True
    ...
    def stop_timers(self):
        ...
        if self.running:
            self.running = False
            with self.condition:
                self.condition.notify()  # Wake up potentially waiting threads
            
            # Wait for worker threads to complete
            if self.worker_thread and self.worker_thread.is_alive():
                self.worker_thread.join(timeout=1.0)  # Wait for up to 1 second
            
            self.worker_thread = None  # Reset thread reference

    def _run(self):
        try:
            # Run WebSocket Server
            self.websocket_server.run_in_thread()

            with Pyro5.api.Proxy("PYRO:PLCObject@localhost:"+str(self.AppFrame.runtime_port)) as proxy:
                """工作线程主循环"""
                while self.running:
                    with self.condition:
                        ...
                        now = time.time()
                        next_time, interval, callback, one_shot, timer_id = self.timers[0]
                        
                        if now < next_time:
                            self.condition.wait(next_time - now)
                            if not self.running:
                                break
                            continue
                        ...
                        if self.timer_ids.get(timer_id, False):
                            ...
                            if not one_shot:
                                ...
                                heapq.heappush(self.timers, (new_time, interval, callback, one_shot, timer_id))
                            else:
                                # Remove One-shot timer after triggering
                                del self.timer_ids[timer_id]
                    ...
                    try:
                        ...
                        if result is not None and result != {}:
                            try:
                                data_json = json.dumps(result, indent=4, default=custom_serializer)
                                # self.logger.write_file(f'{data_json}\n')
                                self.websocket_server.send_message(data_json)
                            except Exception as e:
                                print(f"Send messsage failed: {e}")
                    except Exception as e:
    ...
    def SetAppFrame(self, frame, logger):
        ...
        if self.DispatchDebugValuesTimerId is not None:
            self.remove_timer(self.DispatchDebugValuesTimerId)
        self.DispatchDebugValuesTimerId = None

        # Add status timer
        if frame is not None:
            self.StatusTimerId = self.add_timer(self.PullPLCStatusProc, 0.5, one_shot = False)
    ...
    def ResetAppFrame(self, logger):
        if self.AppFrame is not None:
            ...
            self.remove_timer(self.StatusTimerId)
            self.StatusTimerId = None
            self.AppFrame = None
    ...
    def _Compile_ST_to_SoftPLC(self):
        ...
        self.plcCFLAGS = '"-I%s" -w' % self.iec2c_cfg.getLibCPath()
    ...
    def UpdateMethodsFromPLCStatus(self, proxy=None):
        updated = False
        status = PlcStatus.Disconnected
        PLCstatus = None
        if proxy is not None:
            PLCstatus = proxy.__getattr__('GetPLCstatus')() 
        ...
        if self.previous_plcstate != status:
            ...
            for method, active in list(allmethods.items()):
                self.ShowMethod(method, active)
                self.EnableMethod(method, active)
            self.previous_plcstate = status
            if self.AppFrame is not None:
                updated = True
        return updated
    ...
    def PullPLCStatusProc(self, proxy=None):
        self.UpdateMethodsFromPLCStatus(proxy)
    ...
    #                     #lyl 
    #                     debug_value['debug_tick']=debug_tick
    ...
    #                 with open('debug_value.json', 'w') as json_file:
    #                     json.dump(debug_value, json_file, indent=4, default=custom_serializer)
    #                 print(debug_value)
    ...
    def SnapshotAndResetDebugValuesBuffers(self, proxy=None):
        debug_status = PlcStatus.Disconnected
        debug_values = {}
        # if self.DebugToken is not None:
        #     self.logger.write_file(f'{self.DebugToken}\n')
        if self._connector is not None and self.DebugToken is not None: 
            if proxy is not None:
                debug_status, Traces = proxy.__getattr__('GetTraceVariables')(self.DebugToken)
            else:
                debug_status, Traces =  self._connector.GetTraceVariables(self.DebugToken)         
            # print(Traces)
            #(Traces)
            ...
            if debug_status == PlcStatus.Started:
                if len(Traces) > 0:
                    for debug_tick, debug_buff in Traces:
                        #lyl 
                        debug_values['debug_tick']=debug_tick
                        debug_vars = UnpackDebugBuffer(
                            debug_buff, self.TracedIECTypes)
                        if debug_vars is not None:
                            for IECPath, values_buffer, value in zip(
                            ...
                            if self.LastComplainDebugToken != self.DebugToken :
                                ...
                                self.LastComplainDebugToken = self.DebugToken
                    # with open('debug_value.json', 'w') as json_file:
                    #     json.dump(debug_values, json_file, indent=4, default=custom_serializer)
                    # self.logger.write_file(f'{debug_values}\n')
        ...
        return debug_status, ticks, buffers, debug_values
        ...
        with self.debug_lock:
            ...
            if self._connector is not None and self.debug_status != PlcStatus.Broken:
                ...
                if Idxs:
                    ...
                    self.DebugToken = None
                # self.debug_status, _debug_ticks, _buffers, debug_values = self.SnapshotAndResetDebugValuesBuffers()
        self.DebugUpdatePending = False
    ...
    #         #lyl
    #         buffers=self.RegisterDebugVarToConnector()
    ...
    def _SetDebugData(self,setting):
        if setting["Subscribe"] != None:
            IECPath = setting["Subscribe"]
            self.SubscribeDebugIECVariable(IECPath)
        if setting["Unsubscribe"] != None:
            IECPath=setting["Unsubscribe"]
            self.UnsubscribeDebugIECVariable(IECPath)
        if setting["Force"] != None:
            IECPath,fvalue=setting["Force"]
            self.ForceDebugIECVariable(IECPath,fvalue)
        if setting["Release"] != None:
            IECPath=setting["Release"]
            self.ReleaseDebugIECVariable(IECPath)

        # self.AppendDebugUpdate()
            
    def UpdateDebugVariablesStatus(self):
        attrs={}
        for tuple in self._DbgVariablesList:
            IECPath = tuple['IEC_path']
            idx, iec_type = self._IECPathToIdx[IECPath]
            attrs['IEC_path'] = IECPath
            attrs['idx'] = idx
            attrs['iec_type'] = iec_type
            IECdebug_data = self.IECdebug_datas.get(IECPath, None)
            if IECdebug_data!=None:
                attrs['buffer'] = IECdebug_data[1]
                attrs['variable_status'] = IECdebug_data[2]
                attrs['fvalue'] = IECdebug_data[3]
            else:
                attrs['buffer'] = []
                attrs['variable_status'] = 'Registered'
                attrs['fvalue'] = []
            self.value_attrs[idx] = attrs
            attrs = {}
    ...
    def GetAllVariablesStatus(self):
        self.value_attrs = []
        attrs = {}
        for tuple in self._VariablesList:
            if tuple['vartype'] == 'VAR':
                IECPath = tuple['IEC_path']
                idx, iec_type = self._IECPathToIdx[IECPath]
                attrs['IEC_path'] = IECPath
                attrs['idx'] = idx
    ...
    def GetIECPathToIdx(self):
        return self._IECPathToIdx

    def SubscribeDebugIECVariable(self, IECPath,buffer_list=False):
        """
    ...
    def SubscribeAllDebugIECVariable(self, buffer_list=False):
        """
        Dispatching use a dictionnary linking IEC variable paths
        to a WeakKeyDictionary linking
        weakly referenced callables
        ...
        for tuple in self._DbgVariablesList:
            ...
            #lyl
            #IECdebug_data[0][callableobj] = buffer_list
        # self.ShowDebugVariablesStatus()
        self.AppendDebugUpdate()
    
    # def UpdateAllVarialbes(self):
    #     while True:
    #         # 示例：打印当前时间，表示这个调试线程在持续运行
    #         buffer = self.AppendDebugUpdate()
    #         #print(buffer)
    #         sys.stdout.flush()
    #         # 注意：在实际应用中，应避免无限循环没有退出条件，这里仅为示例
    #         time.sleep(1)  # 暂停5秒，避免过度占用CPU
        

    def UnsubscribeDebugIECVariable(self, IECPath):
    ...
    def UnsubscribeAllDebugIECVariable(self):
        '''Ubsubscribe all variables'''
        self.IECdebug_datas = {}

        self.AppendDebugUpdate()

    def ForceDebugIECVariable(self, IECPath, fvalue):
        '''Force-set a variable's value'''
    ...
    def ReleaseDebugIECVariable(self, IECPath):
        '''Release forced variable assignment'''
        if IECPath not in self.IECdebug_datas:
            return
    ...
    def DispatchDebugValuesProc(self, proxy=None):
        '''Data dispatch thread callback'''
        with self.debug_lock:
            start_time = time.time()
            self.debug_status, debug_ticks, buffers, debug_values = self.SnapshotAndResetDebugValuesBuffers(proxy)
        ...
        self.DispatchDebugValuesTimerId = self.add_timer(self.DispatchDebugValuesProc, int(next_refresh), one_shot = True)

        return debug_values

    
    # def _DispatchDebugValuesProc(self):
    #     start_time = time.time()
    ...
    # def DispatchDebugValuesProc(self):
    #     start_time = time.time()
    #     self.debug_status, debug_ticks, buffers = self.SnapshotAndResetDebugValuesBuffers()
    ...
    #     delay = time.time() - start_time
    #     next_refresh = max(REFRESH_PERIOD - delay, 0.2 * delay)
    #     # self.DispatchDebugValuesTimer = threading.Timer(int(next_refresh), self.DispatchDebugValuesProc)
    ...
    def _connect_debug(self):
        ...
        # Data Dispatch timer
        self.DispatchDebugValuesTimerId = self.add_timer(self.DispatchDebugValuesProc, int(REFRESH_PERIOD), one_shot = True)
        self.start_timers()
    ...
    def _Run(self):
        ...
        if self.GetIECProgramsAndVariables():
            ...
            # self.SubscribeAllDebugIECVariable()
            self._connect_debug()
            success = True
        else:
    ...
    def _SetConnector(self, connector, update_status=True):
        ...
        if connector is not None:
            if self.StatusTimerId is not None:
    ...
    StatusMethods = [
        ...
        {
            "bitmap":    "SetDebugData",
            "name":    _("Set debug data"),
            "tooltip": _("Set debug data"),
            "method":   "_SetDebugData",
            "shown":      False,
        },
        {
            "bitmap":    "Repair",
    ...
    def CallMethod(self, method, args = None):
        for d in self.StatusMethods:
            if d["method"] == method and d.get("enabled", True):
                    if args is None:
                        getattr(self, method)()
                    else:
